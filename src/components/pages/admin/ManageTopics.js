...
import { Col, Row, ListGroup, Button, Table, Modal, Form } from 'react-bootstrap';
        ...
        this.setState({
          ...
          <>
            <Modal.Dialog style={{ backgroundColor: '#B73225' }}>
              ...
              <Modal.Body style={{ textAlign: 'center', color: 'white', fontSize: '2rem', backgroundColor: '#B73225' }}>
                <p style={{ textAlign: 'center', color: 'white', fontSize: '1.1rem' }}>You will be able to Reactivate through Deactivate button above !! </p>
              </Modal.Body>

              <Modal.Footer style={{ backgroundColor: '#B73225' }}>
                <Button onClick={() => this.setState({ confirmDeleteDeactivate: '' })} variant="secondary">CANCEL</Button>
                <Button onClick={() => this.handleDeactivate(id)} variant="primary">CONFIRM DEACTIVATE</Button>
              </Modal.Footer>
            </Modal.Dialog>
        </>,
        })
        ...
        Axios.patch(`http://localhost:5000/api/v2/topic/${id}`, Deactivate, config)
          .then((res) => {
            this.setState({
              ...
              <Modal.Dialog >
                <Modal.Header style={{ textAlign: 'center', color: 'white', fontSize: '2rem', backgroundColor: '#42ba96' }} >
                  <Modal.Title> Topic Successfully Deactivated</Modal.Title>
                </Modal.Header>
              </Modal.Dialog>
          </>,
              topics: [...this.state.topics.filter((topic) => topic.id !== id)],
        ...
        this.setState({
          ...
          <>
            <Modal.Dialog style={{ backgroundColor: '#4da8da' }}>
              <Modal.Header style={{ textAlign: 'center', color: 'white', fontSize: '1.1rem', backgroundColor: '#4da8da' }}>
                    <Form>
            ...
            <LogoutHeader />
                ...
                <div style={{ position: 'fixed', top: 5, left: 5, right: 5, zIndex: 2 }}> {this.state.showMessage}</div>
                    <div className="mb-2">
                        ...
                        <Button style={{ background: '#757575', border: 'none' }} action href="/ManageDiactivatedTopics" size="md">
                        Manage Deactivated Topic
                        </Button>
                    </div>
                    <div style={{ zIndex: 2 }}> {this.state.openCreateTopicForm}</div>
                    <h2 style={{ textAlign: 'center', fontFamily: 'roboto', fontSize: '1.3rem', overflow: 'hidden', padding: '10px', backgroundColor: '#4da8da', color: 'white' }}> ACTIVATED TOPICS</h2>
                    ...
                    {this.state.topics.map((topic) => (
                        <tr>
                            ...
                            <td><Button onClick={(e) => this.handleConfirmDeactivate(topic.id, e)} style={{ background: 'none', color: 'green', border: 'none' }} >Deactivate</Button></td>
